create or replace function mimiciii.sp_calc_sepsisonset_time() returns void
	language plpgsql
as $$
DECLARE
  cur_cohort CURSOR FOR
  SELECT coh.icustay_id,coh.hadm_id,coh.suspected_infection_time_poe, coh.intime,coh.outtime
  ...
  LOOP
    -- fetch row into the film
     fetch next from cur_cohort
      into cv_icustay_id, cv_hadm_id, cv_suspected_infection_time,cv_intime,cv_outtime;
        ...
        from (
          ...
          , echo2 as(
            ...
            left join echodata echo
              ...
              and echo.charttime > current_hour - interval '7' day
          ...
          (
            ...
            inner join inputevents_cv cv
              on ie.icustay_id = cv.icustay_id and cv.charttime between current_hour and next_hour
            left join wt
          ...
          (
            ...
            inner join inputevents_mv mv
              on ie.icustay_id = mv.icustay_id and mv.starttime between current_hour and next_hour
          ...
          (
            ...
            left join ventdurations vd
              ...
              and bg.charttime between current_hour and next_hour
          ...
          --and  ie.icustay_id=cv_icustay_id
          order by ie.icustay_id ) test WHERE test.icustay_id = cv_icustay_id;
