...
Original file is located at
    https://colab.research.google.com/drive/1D6RdyOyNuiEMqBNmn-q1rYTIC2n1nUdy
"""
...
for subject_id in subject_ids:
  ...
  end_time = first_row['TIME'] + datetime.timedelta(minutes = 15)
  ...
  for i in range(skip):
    ...
    end_time = init_time + datetime.timedelta(minutes=15)
...
df_test_null = df_test[df_test.isnull().any(axis=1)]

print(df_features[df_features['SUBJECT_ID']==87577])# --270 2139-08-02 10:33:00

print(df_features[df_features['SUBJECT_ID']==44827])
...
for i in range(5):
    ...
    x_train_subjects, x_test_subjects, y_train_class, y_test_class = train_test_split(x, y, test_size=0.3,shuffle=True) # shuffle and split
    ...
    seq_array_main_back_to_3d = X_oversample.reshape(a_oversmp,4,41) # 3, 41
    ...
    #eval_set = [(seq_array_main_2d, label_array_main_1d)]



    xgb_class = XGBClassifier(learning_rate=0.02, n_estimators=160, max_depth=7, # <- best
                            min_child_weight=7, gamma=0.2, subsample=1, colsample_bytree = 1,
                            objective='binary:logistic', nthread=7, scale_pos_weight= 7, seed=27)

    #alg = XGBClassifier()
    xgb_model = xgb_class.fit(X_oversample, y_oversample ,eval_metric=eval_metrics,eval_set=eval_set) 
    ...
    if __name__ == '__main__':
      ...
      for i in subject_ids_test : #[87481] : #
        ...
        test_df = df_features_classification_test_patient_id[df_features_classification_test_patient_id['SUBJECT_ID']==i]
        ...
        if test_df.shape[0] >= (sequence_length + 1): 
          ...
          number_hours_between_Sepsis_shock = (test_df.HOURS_BETWEEN_SEPSIS_SHOCK.unique())/4
          ...
          if str(shock_onset) != 'nan': 
            print('IS SHOCK')
            has_shock_indv = 1
            count_test_shock = count_test_shock + 1
          else:
            print('NOT SHOCK')
            has_shock_indv = 0
            count_test_non_shock = count_test_non_shock + 1
...
x_train_reg = x.apply(pd.to_numeric)
x_train_reg = np.asarray(x_train_reg)

y_train_reg = y.apply(pd.to_numeric)
y_train_reg = np.asarray(y_train_reg)


glm = sm.GLM(y_train_reg, x_train_reg_best_corr, families= sm.families.Poisson() )
...
for i in subject_ids_test:
  ...
  'HR_MAX', 'RESP_MAX', 'SPO2_MAX', 'TEMP_MAX', 'ABPSYS_MAX', 'ABPDIAS_MAX', 'ABPMEAN_MAX']]
    ...
    #select_X_test = selection.transform(x_test_reg_patient_id)
    #x_test_reg_patient_id = glm_Reg_model.predict(x_test_reg_patient_id)
    
    x_test_reg_patient_id = x_test_reg_patient_id[['ABPSYS', 'ABPDIAS', 'ABPMEAN', 'TEMP', 'HR_MIN', 'SPO2_MIN', 'ABPSYS_MIN', 
                                                   'ABPDIAS_MIN', 'ABPMEAN_MIN', 'RESP_MAX', 'SPO2_MAX', 'TEMP_MAX', 'ABPSYS_MAX', 
                                                   'ABPDIAS_MAX', 'ABPMEAN_MAX']]
...
x_feat_imp1 = df_features[['HR', 'RESP', 'ABPSYS', 'ABPDIAS', 'ABPMEAN', 'SPO2', 'TEMP',
 'HR_STD', 'RESP_STD', 'ABPSYS_STD', 'ABPDIAS_STD', 'ABPMEAN_STD', 'SPO2_STD', 'TEMP_STD', 
 'HR_ENT', 'RESP_ENT', 'ABPSYS_ENT', 'ABPDIAS_ENT', 'ABPMEAN_ENT', 'SPO2_ENT', 'TEMP_ENT',  
 'ABPDIAS_HR_CORR', 'RESP_HR_CORR', 'ABPDIAS_ABPSYS_CORR', 'ABPMEAN_ABPSYS_CORR', 'ABPMEAN_ABPDIAS_CORR', 
 'HR_MIN', 'RESP_MIN', 'SPO2_MIN', 'TEMP_MIN','ABPSYS_MIN', 'ABPDIAS_MIN', 'ABPMEAN_MIN', 
...
x_feat_imp = x_feat_imp1.apply(pd.to_numeric)

y_feat_imp1 = df_features['SOFA_SCORE']
y_feat_imp = y_feat_imp1.apply(pd.to_numeric)
...
for i in range(len(fs.scores_)):
  if fs.scores_[i] >= 50:
    ...
    print('Feature %d: %f' % (i, fs.scores_[i]))
"""
print('Selectinh top features: ', count_best)

x_train_reg_best_corr = SelectKBest(score_func=mutual_info_regression, k=15).fit_transform(x_feat_imp, y_feat_imp)

selector = SelectKBest(score_func=mutual_info_regression, k=15)
selector.fit(x_feat_imp1, y_feat_imp1)
# Get columns to keep and create new dataframe with those only
cols = selector.get_support(indices=True)
features_df_new = x_feat_imp1.iloc[:,cols]
print(features_df_new.columns)

#below two cells for xg boost experiments
# all features normal
x_train_reg_exp = x_train_reg[['ABPSYS', 'ABPDIAS', 'ABPMEAN', 'TEMP', 'HR_MIN', 'SPO2_MIN', 'ABPSYS_MIN', 
                                                   'ABPDIAS_MIN', 'ABPMEAN_MIN', 'RESP_MAX', 'SPO2_MAX', 'TEMP_MAX', 'ABPSYS_MAX',
                                                   'ABPDIAS_MAX', 'ABPMEAN_MAX']]
...
xgbmodel.fit(x_train_reg_exp, y_train_reg)
...
for i in subject_ids_test:
  ...
  'HR_MAX', 'RESP_MAX', 'SPO2_MAX', 'TEMP_MAX', 'ABPSYS_MAX', 'ABPDIAS_MAX', 'ABPMEAN_MAX']]
    ...
    reg_predictions = xgbmodel.predict(x_test_reg_patient_id)
