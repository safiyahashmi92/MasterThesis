...
Original file is located at
    https://colab.research.google.com/drive/125XcV3RH7c6qvia1V-IE4XlfWB6GB_it
"""
...
for subject_id in subject_ids: 
  ...
  if str(shock_onsetttime) == 'nan':
    ...
    q, r = divmod(duration_in_s, 3600) #900
  ...
  end_time = first_row['TIME'] + datetime.timedelta(minutes =60) #15
  ...
  for i in range(skip):
    ...
    if i == 0:
      ...
      end_sofa = df_subset['SOFA_SCORE'].iloc[59] # 14

    #print('start_sofa : ', start_sofa)
    #print('end_sofa :', end_sofa)
    #print((sofa_mean - start_sofa  ))
    
    if (sofa_mean - start_sofa >= 0 )and  (sofa_mean - start_sofa <= 2 ) :
      time_has_shock = 1
    elif sofa_mean - start_sofa > 2 :
      time_has_shock = 2
    else:
      time_has_shock = 0
...
for i in range(5):
    ...
    #over = SMOTE(sampling_strategy=strategy) # 0.01
    under = RandomUnderSampler(sampling_strategy=strategy)# 1
    ...
    """
    alg = XGBClassifier(objective='multi:softmax',learning_rate=0.02, n_estimators=160, max_depth=7, # <- best
                            min_child_weight=7, gamma=0, subsample=1, colsample_bytree = 1,
                            nthread=7,scale_pos_weight= 7,  seed=27, reg_alpha = 0.7)#scale_pos_weight= 7,


    xgb_model = alg.fit(seq_array_main_2d, label_array_main_1d, eval_metric=eval_metrics,eval_set=eval_set) 
    ...
    if __name__ == '__main__':
      ...
      for i in subject_ids_test : #[87481] : #
        ...
        if test_df.shape[0] >= (sequence_length + 1): 
          ...
          """
          if has_shock_indv == 1  :
            array_get_index = numpy.asarray(y_pred)
            itemindex = numpy.where(array_get_index==1)
            try:
              index = itemindex[0][0]
