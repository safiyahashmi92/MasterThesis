# -*- coding: utf-8 -*-
"""1_Fetching Subject IDs for whom waveform records exist.ipynb
...
# To generate a CSV file consisting for only the SEPSIS patients subject_ids from selected cohort for which the waveform exists in the mimic matched waveform db
query_waveform_exists_sepsis = "select subject_id,icustay_id,sepsis_onsettime,intime,outtime from sepsis3_cohort coh where coh.waveform_exists = 1 and coh.has_sepsis=1";
df_waveform_exists_sepsis = pd.read_sql(query_waveform_exists_sepsis,conn);
print(df_waveform_exists_sepsis.shape[0]);

df_waveform_exists_sepsis.to_csv ('new_df_waveform_exists.csv', sep=',', index = False, header=True);