# -*- coding: utf-8 -*-
"""34_TRYING_LSTM_TRIAL.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1555AtRrR9No7ydIlo9fXFASorAkMNXmg
"""
...
for subject_id in subject_ids:
  ...
  if str(shock_onsetttime) == 'nan':
    ...
    #hours_between = ( datetime.datetime.strptime(shock_onsetttime,'%Y-%m-%d %H:%M:%S')  + datetime.timedelta(hours=10) ) - datetime.datetime.strptime(sepsis_onsettime,'%Y-%m-%d %H:%M:%S') 
  ...
  for i in range(skip):
    ...
    #if (has_shock == 1 ) & ( (init_time <= datetime_shock )&  (end_time >= datetime_shock)) :
      #print('in the shock interval')
    """
    if (has_shock == 1 ) and ( ( (init_time + datetime.timedelta(hours =1) )  <= datetime_shock )&  ( ( end_time + datetime.timedelta(hours =1) ) >= datetime_shock)) :
      ...
      time_has_shock = 1
    elif (has_shock == 1 ) and  ( init_time    >= datetime_shock ) :
      time_has_shock = 1
    else:
      time_has_shock = 0
    """
...
"""
# x and y for regression
x_test_reg = x_test[['HR', 'RESP', 'ABPSYS', 'ABPDIAS', 'ABPMEAN', 'SPO2', 'TEMP',
  'HR_STD', 'RESP_STD', 'ABPSYS_STD', 'ABPDIAS_STD', 'ABPMEAN_STD', 'SPO2_STD', 'TEMP_STD', 
  'HR_ENT', 'RESP_ENT', 'ABPSYS_ENT', 'ABPDIAS_ENT', 'ABPMEAN_ENT', 'SPO2_ENT', 'TEMP_ENT',  
  'ABPDIAS_HR_CORR', 'RESP_HR_CORR', 'ABPDIAS_ABPSYS_CORR', 'ABPMEAN_ABPSYS_CORR', 'ABPMEAN_ABPDIAS_CORR', 
  'HR_MIN', 'RESP_MIN', 'SPO2_MIN', 'TEMP_MIN','ABPSYS_MIN', 'ABPDIAS_MIN', 'ABPMEAN_MIN', 
...
subject_ids_test = x_test.SUBJECT_ID.unique()
...
Mean RMSE :  1.6969340848314818
Mean R2 :  -0.38529935570574264
Mean RSS :  143.48343433802657



0.015:

Mean RMSE :  1.6949784707737245
Mean R2 :  -0.3703075090364724
Mean RSS :  145.14243246870183


0.017:

Mean RMSE :  1.7105260826169857
Mean R2 :  -0.3997011923933831
Mean RSS :  146.8979223338939


0.011:
Mean RMSE :  1.703561739002129
Mean R2 :  -0.4042554916568541
Mean RSS :  144.9692161214379

"""

"""# **training the classification model**"""

subject_ids_train = x_train.SUBJECT_ID.unique()
print(len(subject_ids_train)) # 164 for 3 sequence length
...
if __name__ == '__main__':
  ...
  'HR_MAX', 'RESP_MAX', 'SPO2_MAX', 'TEMP_MAX', 'ABPSYS_MAX', 'ABPDIAS_MAX', 'ABPMEAN_MAX']
    ...
    subject_ids_train = x_train_subjects.subject_id.unique()
    for i in subject_ids_train : # [41782] : [86984 , 41782]:
      print(i)
      try:
      ...
      train_df = df_features[df_features['SUBJECT_ID']==i]
      ...
      if train_df.shape[0] >= (sequence_length + 1): 
        ...
        #label_encoding = pd.get_dummies(train_df.HAS_SHOCK)---comented
        
        #train_df = pd.concat([train_df, label_encoding], axis=1)---comented
        
        """ ---comented
        if ( label_encoding.shape[1] == 1 ) & ('Nonshock' in train_df.columns) :
          #print('ADDING SHOCK COLUMN')
          train_df['shock'] = 0

        elif ( label_encoding.shape[1] == 1 ) & ('shock' in train_df.columns) :
          #print('ADDING NON SHOCK COLUMN ')
          train_df['Nonshock'] = 0
        
        
        train_df = train_df[['TIME','ICUSTAY_ID','SUBJECT_ID','SEPSIS_ONSETTIME','SEPSIS_SHOCK_ONSETTIME','HAS_SHOCK','HOURS_BETWEEN_SEPSIS_SHOCK',
  'HR', 'RESP', 'ABPSYS', 'ABPDIAS', 'ABPMEAN', 'SPO2', 'TEMP','SOFA_SCORE',
  'HR_STD', 'RESP_STD', 'ABPSYS_STD', 'ABPDIAS_STD', 'ABPMEAN_STD', 'SPO2_STD', 'TEMP_STD', 
  'HR_ENT', 'RESP_ENT', 'ABPSYS_ENT', 'ABPDIAS_ENT', 'ABPMEAN_ENT', 'SPO2_ENT', 'TEMP_ENT',  
  'ABPDIAS_HR_CORR', 'RESP_HR_CORR', 'ABPDIAS_ABPSYS_CORR', 'ABPMEAN_ABPSYS_CORR', 'ABPMEAN_ABPDIAS_CORR', 
  'HR_MIN', 'RESP_MIN', 'SPO2_MIN', 'TEMP_MIN','ABPSYS_MIN', 'ABPDIAS_MIN', 'ABPMEAN_MIN', 
  'HR_MAX', 'RESP_MAX', 'SPO2_MAX', 'TEMP_MAX', 'ABPSYS_MAX', 'ABPDIAS_MAX', 'ABPMEAN_MAX','Nonshock', 'shock' ]]
        """
...
# Create model
# number of features
nb_features = seq_array_main.shape[2]
#print('nb_features : ' , nb_features)
# number of classes
nb_out = label_array_main.shape[1]
...
# fit the network
history = model.fit(seq_array_main, label_array_main, epochs=100, batch_size=10, verbose=1, shuffle=False #, class_weight=class_weight
...
"""# **Performing regression on the test dataset**"""

#performing regression on entire test dataset:
x_test_reg_all_patients = x_test[test_reg_cols]
print(x_test_reg_all_patients.shape)

select_X_test = selection.transform(x_test_reg_all_patients)

reg_predictions = selection_model.predict(select_X_test)

y_test_reg_all_patient = x_test['SOFA_SCORE']
...
rms = np.sqrt(mean_squared_error(y_test_reg_all_patient, reg_predictions))
print('ROOT MEAN SQUARE ERROR : ',rms )
  
rss = ((y_test_reg_all_patient - reg_predictions)**2).sum()
print("RSS = ", rss)

R2 = sklearn.metrics.r2_score(y_test_reg_all_patient,reg_predictions)
print("R2 = ",R2)

"""# **Performming classification on test dataset**"""
...
from sklearn.preprocessing import StandardScaler
from sklearn.metrics import confusion_matrix, recall_score, precision_score, accuracy_score, f1_score, classification_report, balanced_accuracy_score
from tensorflow.python.keras.models import Sequential, load_model
import tensorflow as tf
   ...
   # pick the feature columns
    ...
    for i in subject_ids_test : 
      ...
      if test_df.shape[0] >= (sequence_length + 1): 
        ...
        if ( label_encoding.shape[1] == 1 ) & ('Nonshock' in test_df.columns) :
          #print('ADDING SHOCK COLUMN')
          has_shock_indv = 0
          test_df['shock'] = 0

        elif ( label_encoding.shape[1] == 1 ) & ('shock' in test_df.columns) :
          #print('ADDING NON SHOCK COLUMN ')
          has_shock_indv = 1
          test_df['Nonshock'] = 0
        
        
        test_df = test_df[['TIME','ICUSTAY_ID','SUBJECT_ID','SEPSIS_ONSETTIME','SEPSIS_SHOCK_ONSETTIME','HAS_SHOCK','HOURS_BETWEEN_SEPSIS_SHOCK',
          'HR', 'RESP', 'ABPSYS', 'ABPDIAS', 'ABPMEAN', 'SPO2', 'TEMP','SOFA_SCORE',
          'HR_STD', 'RESP_STD', 'ABPSYS_STD', 'ABPDIAS_STD', 'ABPMEAN_STD', 'SPO2_STD', 'TEMP_STD', 
          'HR_ENT', 'RESP_ENT', 'ABPSYS_ENT', 'ABPDIAS_ENT', 'ABPMEAN_ENT', 'SPO2_ENT', 'TEMP_ENT',  
          'ABPDIAS_HR_CORR', 'RESP_HR_CORR', 'ABPDIAS_ABPSYS_CORR', 'ABPMEAN_ABPSYS_CORR', 'ABPMEAN_ABPDIAS_CORR', 
        ...
        #label_gen_test = gen_labels(test_df, sequence_length, ['Nonshock', 'shock'])----commmmented
    ...
    print('total test patients : ', count_test)
...
Count of labels for true labels
[2198    0]
 [  23    0]]
TN :  2198
FP :  0
...
df_test_results_indiv = pd.DataFrame(columns= ['SUBJECT_ID','HAS_SHOCK', 'HOURS_BETWEEN_SEPSIS_SHOCK','ACCURACY','FSCORE','PRECISION','RECALL'])
...
print(df_test_results_indiv.sort_values("ACCURACY"))

"""# **below code only for classificaiton**
# **just for reference, do not run**
"""

#print(df_features.SUBJECT_ID.unique())
print(df_features[df_features['SUBJECT_ID']==89091]) # 89091 ,72196
...
if __name__ == '__main__':
   ...
   # pick the feature columns
    ...
    sequence_cols_all = ['TIME','ICUSTAY_ID','SUBJECT_ID','SEPSIS_ONSETTIME','SEPSIS_SHOCK_ONSETTIME','HAS_SHOCK','HOURS_BETWEEN_SEPSIS_SHOCK',
               ...
               'ABPMEAN_ABPSYS_CORR',
               'ABPMEAN_ABPDIAS_CORR']
    for i in [41782] : #subject_ids_train : # [41782] : [86984 , 41782]:
      print(i)
      try:
        train_df.drop(train_df.index, inplace = True)
      except:
      ...
      if train_df.shape[0] >= (sequence_length + 1): 
        ...
        # here I am adding a label based on the sofa_Score
        train_df['HAS_SHOCK'] = np.where(train_df.HAS_SHOCK == 1, 'shock','Nonshock')
        ...
        label_encoding = pd.get_dummies(train_df.HAS_SHOCK)
        ...
        train_df = train_df[['TIME', 'ICUSTAY_ID', 'SUBJECT_ID', 'SEPSIS_ONSETTIME', 'SEPSIS_SHOCK_ONSETTIME', 'HAS_SHOCK', 'HOURS_BETWEEN_SEPSIS_SHOCK', 'HR', 'RESP', 'ABPSYS', 'ABPDIAS', 'ABPMEAN', 'SPO2', 'TEMP', 'SOFA_SCORE', 'ABPDIAS_HR_CORR', 'RESP_HR_CORR', 'ABPDIAS_ABPSYS_CORR', 'ABPMEAN_ABPSYS_CORR', 'ABPMEAN_ABPDIAS_CORR','Nonshock', 'shock' ]]
...
nb_out = label_array_main.shape[1]
print('nb_out : ' , nb_out)
print('label_array_main : ', label_array_main)
...
# fit the network
history = model.fit(seq_array_main, label_array_main, epochs=50, batch_size=16, verbose=1, shuffle=False, class_weight=class_weight
                        # ,validation_split=0.05,
                        # callbacks = [tf.keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=0, patience=5, verbose=0, mode='min')]
                        # ,tf.keras.callbacks.ModelCheckpoint(model_path,monitor='val_loss', save_best_only=True, mode='min', verbose=0)]
                        )
...
if __name__ == '__main__':
   ...
   # pick the feature columns
    ...
    sequence_cols_all = ['TIME','ICUSTAY_ID','SUBJECT_ID','SEPSIS_ONSETTIME','SEPSIS_SHOCK_ONSETTIME','HAS_SHOCK','HOURS_BETWEEN_SEPSIS_SHOCK',
               ...
               'ABPMEAN_ABPSYS_CORR',
               'ABPMEAN_ABPDIAS_CORR']
    for i in [41782] :#subject_ids_test : 
      print(i)
      try:
        test_df.drop(test_df.index, inplace = True)
      except:
      ...
      if test_df.shape[0] >= (sequence_length + 1): 
        ...
        # here I am adding a label based on the sofa_Score
        test_df['HAS_SHOCK'] = np.where(test_df.HAS_SHOCK == 1, 'shock','Nonshock')
        ...
        label_encoding = pd.get_dummies(test_df.HAS_SHOCK)
        
        #print(label_encoding)
        test_df = pd.concat([test_df, label_encoding], axis=1)
        ...
        test_df = test_df[['TIME', 'ICUSTAY_ID', 'SUBJECT_ID', 'SEPSIS_ONSETTIME', 'SEPSIS_SHOCK_ONSETTIME', 'HAS_SHOCK', 'HOURS_BETWEEN_SEPSIS_SHOCK', 'HR', 'RESP', 'ABPSYS', 'ABPDIAS', 'ABPMEAN', 'SPO2', 'TEMP', 'SOFA_SCORE', 'ABPDIAS_HR_CORR', 'RESP_HR_CORR', 'ABPDIAS_ABPSYS_CORR', 'ABPMEAN_ABPSYS_CORR', 'ABPMEAN_ABPDIAS_CORR','Nonshock', 'shock' ]]
        
        """
        if label_encoding.shape[1] == 1:
          has_shock_indv = 0
          #print('INNN NON SHOCK')
          test_df['shock'] = 0
        """
...
print(y_true_list_new)

y_pred_list_new  = numpy.concatenate( y_pred_list, axis=0 )
print(y_pred_list_new)
...
"""
with 6 hours adavcne marking the label for shock = 1 6 hours before the shock onset
0.75, 0.25
...
FN :  98
TP :  11
ACCURACY :  0.912463768115942
PRECISION :  0.171875
RECALL :  0.10091743119266056
F-SCORE :  0.12716763005780346
BALANCED ACCURACY :  0.5340602007448452
...
"""# **TRYING xgbosst for CLASSIFICATION**"""
...
seq_array_main_back_to_3d = X_oversample.reshape(a_oversmp,5,41) # 3, 41
...
alg = XGBClassifier(learning_rate=0.1, n_estimators=140, max_depth=5,
                        ...
                        objective='binary:logistic', nthread=7,  seed=27) #  scale_pos_weight=58,



xgb_model = alg.fit(seq_array_main_2d, label_array_main, eval_metric='auc')
