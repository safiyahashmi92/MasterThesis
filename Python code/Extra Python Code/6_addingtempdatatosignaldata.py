# -*- coding: utf-8 -*-
"""6_AddingTempDataToSignalData.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1R0CCYehKjeQw5k56uKuoaKhGu_pdGUyM
"""
...
df_ALL1655_tempdata = pd.read_csv(io.BytesIO(uploaded['df_ALL1655_tempdata.csv']))

df_ALL1655_tempdata['CHARTTIME'] =  pd.to_datetime(df_ALL1655_tempdata['CHARTTIME'])
print(df_ALL1655_tempdata)

subject_ids_404 = df_all404_csvdata.SUBJECT_ID.unique()
print(subject_ids_404)
print(len(subject_ids_404))

df_all404_csvdata['TEMP'] = np.nan;
...
for subject_id in subject_ids_404:
  df_all404_csvdata_subjectid = df_all404_csvdata[df_all404_csvdata['SUBJECT_ID'] == subject_id]
  for index, row in (df_ALL1655_tempdata[df_ALL1655_tempdata['SUBJECT_ID'] == subject_id]).iterrows():
    #print(row['CHARTTIME'],row['VALUE']) 
    try:
      df_all404_csvdata_index = df_all404_csvdata_subjectid.index[(df_all404_csvdata_subjectid['TIME'].dt.strftime('%Y-%m-%d %H:%M') == (row['CHARTTIME'].strftime('%Y-%m-%d %H:%M' ))) ]
      #print(df_all404_csvdata_index)
      df_all404_csvdata.loc[df_all404_csvdata_index,'TEMP'] = row['VALUE']
...
df_test_52264 = df_all404_csvdata[df_all404_csvdata['SUBJECT_ID']==52264]
print(df_test_52264[['TIME','TEMP','HR']])

df_all404_csvdata.to_csv ('df_ts_records_all404_patients_withTEMP.csv', sep=',', index = False, header=True);

!cp "/content/df_ts_records_all404_patients_withTEMP.csv" "/content/gdrive/My Drive/Master thesis"